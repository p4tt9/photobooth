...
class PhotoboothConfiguration implements ConfigurationInterface
{
    ...
    protected function addDebug(): NodeDefinition
    {
        return (new TreeBuilder('debug'))->getRootNode()->addDefaultsIfNotSet()
            ->ignoreExtraKeys()
            ->children()
            ->booleanNode('telegram')->defaultValue(true)->end()
            ->enumNode('telegram_debug_level')->values(['info', 'debug', 'error'])->defaultValue('error')->end()
            ->scalarNode('telegram_api_token')->defaultValue('7624684504:AAHj3sqb-bAPCFKkpfR8E5MDaFEyRRFJ9xs')->end()
            ->scalarNode('telegram_chat_id')->defaultValue('7192965685')->end()
            ->end();
    }
